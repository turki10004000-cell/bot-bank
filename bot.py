...
def update_store_prices():
    ...
    if time.time() - store_last_update >= 7200:
...
class LoanSelect(discord.ui.Select):
    def __init__(self, requested_amount, user_id):
        ...
        options = [
            discord.SelectOption(label="5k", description="سحب من رصيدك 5k يومياً حتى الانتهاء"),
            discord.SelectOption(label="10k", description="سحب من رصيدك 10k يومياً حتى الانتهاء"),
            discord.SelectOption(label="20k", description="سحب من رصيدك 20k يومياً حتى الانتهاء"),
            discord.SelectOption(label="50k", description="سحب من رصيدك 50k يومياً حتى الانتهاء"),
            discord.SelectOption(label="100k", description="سحب من رصيدك 100k يومياً حتى الانتهاء"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.requested_amount > selected_val:
            ...
            await interaction.response.edit_message(content=f"**سيتم سحب ({selected_val}$) يومياً حتى الانتهاء**", view=None)
...
async def transfer(ctx, member: discord.Member = None, amount: int = 0):
    ...
    if amount < 1000:
        await ctx.send("**لا يقدر يحول أقل من 1000**")
        return

    sender = get_user(ctx.author.id)
    if sender["balance"] < amount:
        await ctx.send("**بس يا الغني أنت ما معك المبلغ**")
        return
...
async def top(ctx):
    ...
    embed = discord.Embed(title="**قائمة أغنى أشخاص بالسيرفر**", description=f"### {desc}", color=0x000000)
    await ctx.send(embed=embed)
...
async def thieves(ctx):
    ...
    embed = discord.Embed(title="**قائمة حرامية السيرفر**", description=f"### {desc}", color=0x000000)
    await ctx.send(embed=embed)
...
async def rateb(ctx):
    ...
    embed = discord.Embed(
        description=f"### **إشعار إيداع راتب**\n**الوظيفة : {job}**\n**الراتب : {amount}$**\n**رصيدك الحالي : {user['balance']}$**",
        color=0x000000
    )
    await ctx.send(embed=embed)
...
async def rahan(ctx, member: discord.Member = None, amount: int = 0):
    ...
    if not member or amount < 5000:
        await ctx.send("**الاستخدام: رهان @الشخص المبلغ (أقل مبلغ 5000)**")
        return
...
async def rob(ctx, member: discord.Member = None):
    ...
    if member == bot.user:
        await ctx.send("**كيف تسرق بوت يا غبي**")
        return
    ...
    if not ok:
        await ctx.send(f"**⌛ بس يا حرامي تعال بعد الوقت المحدد**")
        return

    if victim["shield_until"] > now:
        await ctx.send(f"**⌛ الحماية مفعلة للضحية**")
        return

    if now - victim.get("last_robbed_time", 0) < 300:
        await ctx.send("**المسكين منزرف، انتظر قليلاً**")
        return
    ...
    await ctx.send(f"**تم نهب {stolen_amount}$ بنجاح**")
...
async def play_game(ctx, amount, game_name, cmd_key):
    ...
    if amount is None or amount < 1000:
        await ctx.send("**أقل مبلغ للعب هو 1000$**")
        return
    ...
    if is_win:
        ...
        user["balance"] += profit
        desc = f"### **{game_name} رابح بنسبة {percentage}%**\n**مبلغ الربح : {profit}$**\n**رصيدك السابق : {prev_balance}$**\n**رصيدك الحالي : {user['balance']}$**"
    else:
        loss = int(amount * (percentage / 100))
        user["balance"] -= loss
        if user["balance"] < 0: user["balance"] = 0
        desc = f"### **{game_name} خاسر بنسبة {percentage}% 😜**\n**مبلغ الخسارة : {loss}$**\n**رصيدك السابق : {prev_balance}$**\n**رصيدك الحالي : {user['balance']}$**"
...
async def shield(ctx, hours: int = 0):
    if hours not in [1, 2, 3, 4, 5]:
        await ctx.send("**حط عدد ساعات الحماية من 1 إلى 5**")
        return
...
async def marry(ctx, member: discord.Member = None, amount: int = 0):
    ...
    if member == bot.user:
        await ctx.send("**كيف تبي تتزوج بوت يامتوحد**")
        return

    if not member or amount < 10000:
        await ctx.send("**منشن فتاة أحلامك وحط المهر (أقل مهر 10,000)**")
        return
    ...
    if len(proposer["spouses"]) >= 4:
        await ctx.send("**الشرع حلل أربع بس!**")
        return

    if len(target["spouses"]) >= 1:
        await ctx.send("**متزوجة الله يعوضك**")
        return
    ...
    msg = await ctx.send(f"**عقد زواج**\n**من : {ctx.author.mention} إلى : {member.mention}**\n**المهر : {amount}$**")
...
async def marriages_top(ctx):
    ...
    for idx, (uid, data) in enumerate(sorted_users, 1):
        ...
        desc += f"**#{idx} {name1} 💍 {name2}**\n"

    embed = discord.Embed(title="**قائمة أغلى زواجات بالسيرفر**", description=f"### {desc}", color=0x000000)
    await ctx.send(embed=embed)
...
async def divorce(ctx):
    user = get_user(ctx.author.id)
    if not user["spouses"]:
        await ctx.send("**عزابي أنت بتطلق من**")
        return
...
async def store(ctx):
    ...
    desc = f"**سيتم التحديث خلال : {timer}**\n\n"
    for k, v in store_items.items():
        desc += f"**{k} : {v['current']}$**\n"

    embed = discord.Embed(title="**المتجر**", description=f"### {desc}", color=0x000000)
...
async def inventory(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(title="**الممتلكات**", description=f"### {desc}", color=0x000000)
    await ctx.send(embed=embed)
...
async def status_time(ctx):
    ...
    # أسماء الأوامر بالعربي ومفاتيحها البرمجية
    cmds = {
        "بخشيش": "bakhsheesh",
        "حظ": "haz",
        "راتب": "rateb",
    ...
    desc = ""
    for ar_name, en_key in cmds.items():
        last = user["cooldowns"].get(en_key, 0)
        if now - last >= 300:
            status = "🟢 جاهز للعب"
        else:
            ...
            mins = rem // 60
            secs = rem % 60
            status = f"🔴 غير جاهز للعب ({mins:02d}:{secs:02d})"
        desc += f"**{ar_name} : {status}**\n"

    embed = discord.Embed(title="**حالة الأوقات**", description=f"### {desc}", color=0x000000)
    await ctx.send(embed=embed)
...
async def slash_help(interaction: discord.Interaction):
    desc = "**بخشيش**\n**تحويل**\n**توب**\n**حراميه**\n**حظ**\n**راتب**\n**رهان**\n**قرض**\n**سداد**\n**فلوس**\n**نهب**\n**استثمار**\n**تداول**\n**قمار**\n**حمايه**\n**نرد**\n**زواج**\n**زواجي**\n**زواجات**\n**طلاق**\n**خلع**\n**متجر**\n**شراء**\n**بيع**\n**ممتلكات**\n**وقت**"
    mbed = discord.Embed(title="**قائمة الأوامر**", description=f"### {desc}", color=0x000000)
    await interaction.response.send_message(mbed, ephemeral=True)
